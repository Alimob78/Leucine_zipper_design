...
parser.add_argument( "-runlist", type=int, default=-1, help="The index of the file to be run from the directory for parallel running. Only used when -pdbdir is active (default: ''; Run all PDBs)" )
...
class StructManager():
    ...
    def __init__(self, args):
        ...
        if not args.pdbdir == '':
            ...
            # sort the files to ensure consitency across each new iteration
            self.struct_iterator = sorted(self.struct_iterator)
            ...
            # Parse the runlist and determine which structures to process
            if args.runlist != -1:
                index_to_keep = int(args.runlist)
                if 0 <= index_to_keep < len(self.struct_iterator):
                    # Keep only the structure at the specified index
                    print(self.struct_iterator)
                    self.struct_iterator = [self.struct_iterator[index_to_keep]]
                    print(f'After filtering by runlist, {len(self.struct_iterator)} structure remains')
                else:
                    print(f'Invalid index: {index_to_keep}. The index should be between 0 and {len(self.struct_iterator) - 1}.')
